...
public class EncounterServiceTest {
    ...
    public void shouldReturnErrorEvenIfOneGetEncounterFails() throws ParseException {
        ...
        when(facilityService.ensurePresent("1")).thenReturn(
                Observable.just(new Facility("1", "facility1", "Main hospital", "3056,30", new Address("1", "2", "3", null, null))));

        final String exceptionMessage = "I bombed";

        when(encounterRepository.findEncountersForCatchment(eq(new Catchment("30")), org.mockito.Matchers.any(Date.class), eq(20))).
                thenReturn(Observable.<List<EncounterBundle>>error(new Exception(exceptionMessage)));
        try {
            List<EncounterBundle> encountersByCatchments = encounterService.findEncountersForFacilityCatchment("1", "30", date, 20).toBlocking().first();
        } catch (Exception e) {
