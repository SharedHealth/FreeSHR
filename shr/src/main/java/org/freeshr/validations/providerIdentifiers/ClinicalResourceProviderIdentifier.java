...
public abstract class ClinicalResourceProviderIdentifier {
    ...
    protected abstract boolean canValidate(Resource resource);

    protected abstract List<Reference> getProviderReferences(Resource resource);

    public final boolean isValid(Resource resource, SHRProperties shrProperties) {
        if (!canValidate(resource)) return true;
