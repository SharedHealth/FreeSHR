    ...
    public List<ValidationMessage> validate(ValidationSubject<AtomFeed> subject) {
        ...
        if (compositionEntry == null) {
            validationMessages.add(new ValidationMessage(null, ResourceValidator.INVALID, "Feed",
                    "Feed must have a Composition with an encounter.", OperationOutcome.IssueSeverity.error));
        ...
        for (String entryReferenceId : compositionSectionIds) {
            validationMessages.add(new ValidationMessage(null, ResourceValidator.INVALID, entryReferenceId,
                    String.format("No entry present for the section with id %s", entryReferenceId),
    ...
    private List<String> verifyEntryReferenceIds(List<AtomEntry<? extends Resource>> entryList,
        ...
        for (AtomEntry<? extends Resource> atomEntry : entryList) {
            if (!atomEntry.getResource().getResourceType().equals(ResourceType.Composition)) {
                ...
                validationMessages.add(new ValidationMessage(null, ResourceValidator.INVALID, identifier,
                        String.format("Entry with id %s is not present in the composition section list.",
